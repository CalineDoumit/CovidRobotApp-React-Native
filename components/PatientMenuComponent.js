import React, { Component } from 'react';
import { View, Text, TouchableOpacity, ScrollView, Image } from 'react-native';
import { TextInput } from 'react-native-gesture-handler';
import { connect } from 'react-redux';
import Icon from 'react-native-vector-icons/FontAwesome';
import { Table, TableWrapper, Row, Cell } from 'react-native-table-component';
import { logoutUser, fetchUsers, fetchPatients } from '../redux/ActionCreators'
import { ThemeProvider } from 'react-native-elements';
import * as SecureStore from 'expo-secure-store';



const mapStateToProps = state => {
    return {
        users: state.users,
        patients: state.patients,
    }
}

const mapDispatchToProps = dispatch => ({
    fetchUsers: () => dispatch(fetchUsers()),
    logoutUser: () => dispatch(logoutUser()),
    fetchPatients: () => dispatch(fetchPatients()),
    fetchRobotGo: (robotId) => { dispatch(fetchRobotGo(robotId)) },
    fetchRobotCome: (robotId) => { dispatch(fetchRobotCome(robotId)) },
    fetchRobotStop: (robotId) => { dispatch(fetchRobotStop(robotId)) },


})



class PatientMenu extends Component {
    constructor(props) {
        super(props);
        this.state = {
            myself: null,
            tableData: [30, 15, 20],
            tableHead: ['Temperature'],
            currentuser: this.props.navigation.getParam('user'),
            currentpatient: null,
        }
        this.handleLogout = this.handleLogout.bind(this);
        this.RobotGo = this.RobotGo.bind(this);
        this.RobotCome = this.RobotCome.bind(this);
        this.RobotStop = this.RobotStop.bind(this);

    }

    static navigationOptions = {
        title: 'Patient Menu',
        headerLeft: null,
    };


    componentWillMount() {
        this.props.fetchUsers();
        this.props.fetchPatients();
        this.props.patients.patients.map((patient) => {
            if (patient._id === this.state.currentuser.patient)
                this.setState({ currentpatient: patient })
        })
    }



    handleLogout = () => {
        this.props.logoutUser();
        this.props.navigation.navigate('Login')
    }

    RobotGo(robotId) {
        console.log("patient ID: " + robotId)
        this.props.fetchRobotGo(robotId);

    }

    RobotCome(robotId) {
        console.log("patient ID: " + robotId)
        this.props.fetchRobotCome(robotId);

    }

    RobotStop(robotId) {
        console.log("patient ID: " + robotId)
        this.props.fetchRobotStop(robotId);
    }




    render() {
        {
            this.state.tableData.length = 0,
                this.state.currentpatient.temperatures.map((temp) => {
                    this.state.tableData.push([temp])
                })
        }

        return (
            <View style={{ backgroundColor: '#FFFFFF' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                    <Image source={{ uri: "data:image/jpeg;base64,'" }}
                        style={{
                            width: 70,
                            height: 70,
                        }}
                    />
                    <Text style={{ fontSize: 30, alignSelf: 'center' }}> ROOM : </Text>
                </View>

                <ScrollView style={{ height: "100%" }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 20 }}>
                        <TouchableOpacity style={{ margin: 5, borderRadius: 30, backgroundColor: '#4ea8cb', width: 80, alignItems: 'center' }}
                            onPress={
                                () => this.RobotGo(this.state.myrobot.number)
                            } >
                            <Text style={{ color: 'white', fontSize: 20 }}>GO</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={{ margin: 5, borderRadius: 30, backgroundColor: '#4ea8cb', width: 80, alignItems: 'center', justifyContent: 'center' }}
                            onPress={() => {
                                this.RobotStop(this.state.myrobot.number);
                            }} >
                            <Text style={{ color: 'white', fontSize: 18 }} >STOP</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={{ margin: 5, borderRadius: 40, backgroundColor: '#4ea8cb', width: 80, alignItems: 'center', justifyContent: 'center' }}
                            onPress={() => {
                                this.RobotCome(this.state.myrobot.number);
                            }} >
                            <Text style={{ color: 'white', fontSize: 18 }} >COME</Text>
                        </TouchableOpacity>
                    </View>
                    <View style ={{flexDirection:'row'}}>
                        <TouchableOpacity style={{ margin: 5, width:200,borderRadius: 40, backgroundColor: '#4ea8cb', alignItems: 'center', justifyContent: 'center' }}
                            onPress={() => {
                                this.props.navigation.navigate("AboutNurses");
                            }} >
                            <Text style={{ color: 'white', fontSize: 18 }} >MEET OUR NURSES</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={{margin: 5,width:100, borderRadius: 40, backgroundColor: '#4ea8cb', alignItems: 'center', justifyContent: 'center' }}

                            onPress={() => {
                                this.handleLogout();
                            }} >

                            <Text style={{ color: 'white', fontSize: 18 }} >LOGOUT</Text>

                        </TouchableOpacity>
                    </View>



                    <View style={{ width: "100%", backgroundColor: '#0099CC', height: 40, justifyContent: 'flex-start', flexDirection: 'row', alignItems: 'center' }} >
                        <Icon name="user" size={30} color='#FFFFFF' />
                        <Text style={{ fontSize: 25, fontWeight: "bold", color: 'white' }}> Patient </Text>
                    </View>
                    <Text style={{ fontSize: 15 }}>{this.state.currentuser.firstname} {this.state.currentuser.lastname} </Text>
                    <View style={{ flexDirection: 'row' }}>
                        <Text style={{ fontSize: 15 }}>{this.state.currentuser.phonenumber} </Text>
                    </View>
                    <Text style={{ marginBottom: 20, fontSize: 15 }}>{this.state.currentpatient.emergencyContact} </Text>


                    <View style={{ marginBottom: 20 }}>
                        <View style={{ width: "100%", backgroundColor: '#0099CC', height: 40, justifyContent: 'flex-start', flexDirection: 'row', alignItems: 'center' }} >
                            <Icon name="hospital-o" size={30} color='#FFFFFF' />
                            <Text style={{ fontSize: 25, fontWeight: "bold", color: 'white' }}> Description </Text>
                        </View>
                        <Text style={{ fontSize: 15 }}>{this.state.currentpatient.description} </Text>
                        <Text style={{ fontSize: 15 }}>{this.state.currentpatient.dateofBirth}</Text>
                        <Text style={{ fontSize: 15 }}>{this.state.currentpatient.allergies}</Text>
                        <Text style={{ fontSize: 15 }}>{this.state.currentpatient.bloodType} </Text>
                    </View>





                    <View style={{ width: "100%", backgroundColor: '#0099CC', height: 40, justifyContent: 'flex-start', flexDirection: 'row', alignItems: 'center', marginBottom: 10 }} >
                        <Icon name="thermometer" size={30} color='#FFFFFF' />
                        <Text style={{ fontSize: 25, fontWeight: "bold", color: 'white' }}> Temperature </Text>
                    </View>

                    <View style={{ width: "100%" }}>
                        <Table borderStyle={{ borderColor: 'black' }}>
                            <ScrollView style={{ height: '70%' }} >
                                {
                                    this.state.tableData.map((rowData, index) => (
                                        <TableWrapper key={index} style={{ flexDirection: 'row', backgroundColor: '##F5FFFA', height: 30 }}>
                                            {
                                                rowData.map((cellData, cellIndex) => (
                                                    <Cell key={cellIndex} data={cellData} textStyle={{ margin: 6, height: 40 }} />
                                                ))
                                            }
                                        </TableWrapper>
                                    ))
                                }
                            </ScrollView>
                        </Table>
                    </View>

                </ScrollView>



            </View>
        )
    }
}



export default connect(mapStateToProps, mapDispatchToProps)(PatientMenu);


